...
/// The primary Tabs container view, consisting of a top header for the tab bar and other elements and a bottom area for tab content.
///
...
/// wrapper `ContainerTabsScroll`. Swiping left and right on tab contents pages between tabs. Each content view must be identified and configured
/// using the `containerTabItem()` view modifier (conceptually similar to a combination of the `tab()` and `tabItem()` view
...
/// as fade, shrink and parallax. The effects are driven by a variety of dynamic metrics, through the stream of `ContainerTabsHeaderContext` values
...
/// unique effects.
///
/// To use sticky headers without tabs, use the `StickyHeader` view instead of `ContainerTabs`.
...
where HeaderTitle: View, HeaderTabBar: View, HeaderBackground: View, Content: View, Tab: Hashable {
    ...
    /// Constructs a tabs container component with a header title, tab bar and tab contents (no background).
    ///
    /// - Parameters:
    ///   - selectedTab: The external tab selection binding.
    ///   - headerTitle: The header title view builder.
    ///   - headerTabBar: The header tab bar. `ContainerTabBar` is typically used, but any custom view may be provided.
    ///   - content: a content view builder, who's top level elements are assumed to be individual tab contents.
    ///
    /// Top-level content elements are typically `ContainerTabsScroll` views. `ContainerTabsScroll` is a lightweight wrapper, around
    /// `ScrollView` and is required to enable scroll effects. Top level content elements must apply the `containerTabItem()` view modifier
    /// in order to identify and configure each tab (conceptually similar to a combination of the `tab()` and `tabItem()` view
    /// modifiers used with a standard `TabView`).
    public init(
        selectedTab: Binding<Tab>,
        @ViewBuilder headerTitle: @escaping (ContainerTabsHeaderContext<Tab>) -> HeaderTitle,
        @ViewBuilder headerTabBar: @escaping (ContainerTabsHeaderContext<Tab>) -> HeaderTabBar,
    ...
    /// Constructs a tabs container component with a tab bar and tab contents (no title or background).
    ///
    /// - Parameters:
    ///   - selectedTab: The external tab selection binding.
    ///   - headerTitle: The header title view builder.
    ...
    /// Constructs a tabs container component with all elements: header title, tab bar, header background and tab contents.
    ///
    /// - Parameters:
    ///   - selectedTab: The external tab selection binding.
    ///   - headerTitle: The header title view builder.
    ...
    public init(
        ...
        @ViewBuilder headerTitle: @escaping (ContainerTabsHeaderContext<Tab>) -> HeaderTitle,
        @ViewBuilder headerTabBar: @escaping (ContainerTabsHeaderContext<Tab>) -> HeaderTabBar,
        @ViewBuilder headerBackground: @escaping (ContainerTabsHeaderContext<Tab>) -> HeaderBackground,
        @ViewBuilder content: @escaping () -> Content
    ) {
    ...
    @ViewBuilder private let header: (ContainerTabsHeaderContext<Tab>) -> HeaderView<HeaderTitle, HeaderTabBar, HeaderBackground, Tab>
                ...
                header(headerModel.state.headerContext)
                    .background {
                        if !headerModel.state.tabsRegistered {
                            ...
                            /// and, due to the laziness, tabs that are off-screen do not get registered. When the same content
