...
/// A context value passed to sticky header components, providing metrics useful for creating sticky header scroll effects.
///
...
/// impression of shrinking, fading, parallax, etc. All of these effects are achieved by manipulating the views based on the `HeaderContext`
...
public struct HeaderContext<Tab>: Equatable where Tab: Hashable {
    ...
    /// height including top safe area. Applying `scaleEffect()` as a scroll effect does not affect this value.
    ...
    /// A value related to sticky titles. See the `minTitleHeight()` view modifier.
